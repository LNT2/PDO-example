...
if (!isset ($link)){
require 'connect.php';
$query = $pdo->prepare("select * from address_book order by last_name, first_name, id");
$query->execute();
...
echo '<td class="small"><input class="button2" type="submit" value="'.$address_book['id'].'" name="delete_id" alt="submit"></td>';
echo '</tr>';
}
} else{
	switch ($link) {
